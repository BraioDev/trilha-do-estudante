...
const requestListener = function (req, res) {
  if (req.url === "/api/salvar" && req.method === "POST") {
    // Método para acessar via postman em: http://localhost:8080/api/salvar
    ...
    req.on("end", () => {
      try {
        ...
        if (data && data.texto) {
          ...
          res.end(
            "Solicitação inválida: o corpo da solicitação deve conter uma propriedade 'texto' em formato JSON"
          );
        }
      } catch (error) {
        res.writeHead(400, { "Content-Type": "text/html; charset=utf-8" });
        res.end(
          "Solicitação inválida: o corpo da solicitação deve ser um JSON válido"
        );
      }
    });
  } else if (req.url === "/api/ler" && req.method === "GET") {
    // Método para acessar via postman em: http://localhost:8080/api/ler
